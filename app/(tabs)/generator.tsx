...
export default function GeneratorScreen() {
  ...
  // Örnek görüntü üretimi simülasyonu
  const generateImage = () => {
    if (!prompt.trim()) return;
    
    setLoading(true);

    // Gerçek uygulamada burada bir AI API'ye istek gönderilir
    // Burada sadece gecikme simülasyonu yapıyoruz
    setTimeout(() => {
      // Örnek bir resim URL'si (gerçek uygulamada AI tarafından üretilen görselin URL'si olacak)
      const mockImageUrl = 'https://picsum.photos/500/500';
      
      setGeneratedImage(mockImageUrl);
      setHistory(prev => [{prompt, image: mockImageUrl}, ...prev].slice(0, 10));
      setLoading(false);
    }, 2000);
  };
